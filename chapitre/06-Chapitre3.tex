...
\begin{figure}[H]   
    \centering
    \includegraphics[width=14cm, height=11cm]{images/architecturephysique3.jpg}
...
\noindent L'architecture logique repose sur le patron MVC avec une séparation claire des responsabilités entre couches. Cette organisation assure la maintenabilité et facilite l'évolution du système.

\noindent La figure 3.13 illustre l'organisation logique de notre application :

\begin{figure}[H]
    \centering
    \includegraphics[width=15cm, height=13cm]{images/architecturelogique3.jpg}
    \label{fig:arch_logique}
    \caption{Architecture logique de l'application}
\end{figure}

\noindent L'utilisateur interagit avec le système via un navigateur qui héberge la couche présentation, implémentée en composants React. Cette interface dialogue avec la frontière applicative (API) qui constitue le point d'entrée logique du système.

\noindent \textbf{Couche Contrôleurs} : Implémentée avec des contrôleurs ASP.NET Core, elle reçoit et valide les requêtes en provenance de la couche présentation. Elle gère l'authentification et l'autorisation basées sur les rôles et orchestre le routage des opérations vers les services métiers. Principales responsabilités : Authentification, Gestion utilisateurs Gestion conformité.

\noindent \textbf{Couche Métier} : Contient les services applicatifs qui encapsulent les règles métier et coordonnent les opérations impliquant plusieurs entités. Cette couche applique le principe de responsabilité unique et reste indépendante des détails de persistance et de présentation.

\noindent \textbf{Couche Accès aux Données} : Implémente l'accès aux données via AuditDbContext (Entity Framework Core). Elle gère la persistance des entités métier et assure la gestion des transactions pour la cohérence des données.

\noindent \textbf{Microservice NLP} : Service autonome (Flask) dédié aux traitements linguistiques. Il expose une frontière de service pour les opérations d'analyse , de suggestion de taxonomie et de génération de rapports de performance. Isoler cette fonctionnalité permet une scalabilité et une évolution indépendantes sans impacter la logique métier principale.

\noindent Cette architecture respecte les principes SOLID et la séparation des préoccupations : faible couplage entre couches, meilleure testabilité et possibilité d'évolution progressive. L'approche microservice est volontairement limitée aux fonctionnalités spécialisées (NLP), le cœur applicatif restant une application monolithique modulée par couches.
\section*{Conclusion} 
